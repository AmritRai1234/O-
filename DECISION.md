...
- [dogfood demo: examples/serve, 2026-08-25] Built a real dev tool WITH o- as the demo: zero-config static file server with directory listing + live reload (the python -m http.server / npx serve replacement). o- new scaffolded it; o- bundle embedded templates/livereload.js; o- test ran 6 tests (serving, injection, traversal 403, listing, reload endpoint, embedded script); o- run gave hot reload; o- build produced a 5.3 MiB single binary that works standalone. DOGFOOD GAP FOUND + FIXED: o- run did not pass args to the app — added `o- run -- <args>` passthrough (runner.StartArgs now used by run.go; Use string documents it).
